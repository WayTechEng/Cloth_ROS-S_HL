...
public class VoiceCommands : MonoBehaviour
{
    ...
    public void SetPointCustomMulti(List<List<Vector3>> list)
    {
        ...
        for (int i = 0; i < x; i++)
        {
            Vector3 pick = list[i][0];
            Vector3 place = list[i][1];
            Transform child_pick = Instantiate(spherePoint, pick, Quaternion.identity);
            Transform child_place = Instantiate(spherePoint, place, Quaternion.identity);
            Vector3 v_pick = ros_world_coord_frame.InverseTransformPoint(child_pick.position);
            Vector3 v_place = ros_world_coord_frame.InverseTransformPoint(child_place.position);
            pathPoints.Add(v_pick + EEF_Offset);
            pathPoints.Add(v_place + EEF_Offset);
